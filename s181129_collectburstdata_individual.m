...
for file_i=1:length(baselineschluessel)
    ...
    numBL{file_i,1}=rs_NumBlockLength;
close all
clearvars -except totalp75_10 numBL totalp75_180 totalbinprob180 histogram_einstellung_norm histogram_einstellung totalbinprob10 files file_i baselineschluessel
end
...
clearvars -except TP101 TP104 TP110 TP116 TP121 TP300 TP400 numBL

meanBL = cellfun(@mean, numBL);
stdBL=cellfun(@std, numBL)./(sqrt(7));
mean101=mean(meanBL(TP101));
mean104=mean(meanBL(TP104));
mean110=mean(meanBL(TP110));
mean116=mean(meanBL(TP116));
mean121=mean(meanBL(TP121));
mean300=mean(meanBL(TP300));
mean400=mean(meanBL(TP400));

std101=mean(stdBL(TP101));
std104=mean(stdBL(TP104));
std110=mean(stdBL(TP110));
std116=mean(stdBL(TP116));
std121=mean(stdBL(TP121));
std300=mean(stdBL(TP300));
std400=mean(stdBL(TP400));

figure
hold on
bar([mean101, mean104, mean110, mean116, mean121, mean300, mean400], 0.8,'FaceColor', 'k', 'EdgeColor', 'k')
errorbar([1:7],[mean101, mean104, mean110, mean116, mean121, mean300, mean400],[0 0 0 0 0 0 0],[std101, std104, std110, std116, std121, std300, std400], '.','Color', 'k')
hold off
