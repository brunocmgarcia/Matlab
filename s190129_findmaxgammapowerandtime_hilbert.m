%% s190129_findmaxgammapowerandtime_hilbert
...
for file_i=10:length(files)
    ...
    cfg=[];
    cfg.bpfilter='yes';
    cfg.bpfreq=[masterpeakfreq(file_i)-2 masterpeakfreq(file_i)+2];
    cfg.bpfiltord=5;
    data=ft_preprocessing(cfg,data);
    cfg=[];
    cfg.hilbert='yes';
    ...
    filteredTFR=data.trial{1,1};
               
       filteredmaxpoweringamma=medfilt1(filteredTFR,50000,[],2,'omitnan');
    ...
    crossing80=(filteredmaxpoweringamma-maxfilteredmaxpoweringamma_80);
     ...
     hold on
     plot(filteredTFR,'Color',[.5 .5 .5])
     plot(filteredmaxpoweringamma,'r')
     fplot(maxline,[1 length(filteredTFR)],'k')
     fplot(fuenfundneunziglin,[1 length(filteredTFR)],'k')
     fplot(zwanziglin,[1 length(filteredTFR)],'k')
     fplot(achziglin,[1 length(filteredTFR)],'k')
    ...
    fplot(peakline,[1 length(filteredTFR)],'b')
