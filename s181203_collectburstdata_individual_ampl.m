...
histogram_einstellungMax=150 %(2500 );%5e+6;
...
for file_i=1:length(baselineschluessel)
    ...
    figure
    subplot(1,2,1)
    myhist180=histogram(AUC, histogram_einstellung,  'Normalization', histogram_einstellung_norm);
    subplot(1,2,2)
    myhist10=histogram(AUCBL, histogram_einstellung,  'Normalization', histogram_einstellung_norm);
    ...
    lengthLD=basestruct.rs_NumBlockLength;
    lengthBL=rs_NumBlockLength;
...
close all
clearvars -except  rate baserate BLaverageAUC LDaverageAUC totalp75_10 totalp75_180 totalbinprob180 histogram_einstellung_norm histogram_einstellung totalbinprob10 files file_i baselineschluessel
end
...
finalfig=figure('Name','Burst Amp Probability Baseline vs. L-Dopa', 'Units','Normalized','Position',[0 0 1 1]);
hold on
...
%errorbar(1:7,mean(totalratedevelopment,1),[],std(totalratedevelopment,1),'k','linestyle','none');
  ...
  for i=1:length(kombiniertrateplot)
        ...
        set(kombiniertrateplot(1,i),'FaceColor','none')
        set(kombiniertrateplot(1,i),'EdgeColor',[0 0 0], 'LineWidth', 1) %farbenkombiniert(i,:)
