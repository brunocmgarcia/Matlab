...
%
% !!!!!!!!!! Manual inputs: Step 1) CG, TPs, LB, Region ; Step 3) cfg.bpfreq,
% cfg.channel    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
for tp_i = 1: length(TPs) 
    clearvars -except TPs tp_i
    cd('C:\Users\bruno\Documents\LR3\4bruno\4bruno\Data')
    CG = 'CG07'; %Input animal number
    TP = TPs{tp_i}; %Input date relative to 6OHDA injection
    ...
    cfg.bpfreq=[13.6786 23.6786]; 
    % center frequency of the highest power among TPs for the peak in the
    % periodic component of fooofed s190122_quickanddirtybetapower (+/- 5),
    % for this Region in this animal
    
    data=ft_preprocessing(cfg,data);
    %after this you have the bandpass filtered data and rereferenced to the cerebellum.
    
    cfg=[];
    cfg.channel= 25; %ch 30 is the M1, ch1-15 is STR, and ch16-29 is SNR 
    %(choose one of the included channels in the pwelch analysis) 
    ...
    if strcmp(TP, 'TP00')
        ...
        [BinNumber_Length,BinEdges_Length] = histcounts(NumBlockLength,[0:0.03:1 inf]);
        BinPercentage_Length = BinNumber_Length./sum(BinNumber_Length);
    else
        ...
        histolength  = histogram (NumBlockLength,[0:0.03:1 inf], 'Normalization', 'probability');
        legend ('TP00', TP)
        %     Bin_Edges = histolength.BinEdges;
        %     Bin_Counts = histolength.BinCounts;
        %     Bin_Values = histolength.Values;
        [BinNumber_Length,BinEdges_Length] = histcounts(NumBlockLength,[0:0.03:1 inf]);
        BinPercentage_Length = BinNumber_Length./sum(BinNumber_Length);
        hold off
    end
    ...
    if strcmp(TP, 'TP00')
        ...
        histoarea  = histogram (AreaUnderCurve,[0:20:1000 inf], 'Normalization', 'probability');
        xlabel ('AUC (a.u.)')
        title ('Distribution of suprathreshold AUC(data) - AUC(percentile)')
        [BinNumber_Area,BinEdges_Area] = histcounts(AreaUnderCurve,[0:20:1000 inf]);
        BinPercentage_Area = BinNumber_Area./sum(BinNumber_Area);
    else
    ...
    betaburst = struct ('NumBlockStart', NumBlockStart, 'NumBlockEnd',NumBlockEnd, 'NumBlockLength', NumBlockLength,...
        ...
        'AreaUnderCurve', AreaUnderCurve, 'BinNumber_Length', BinNumber_Length, 'BinEdges_Length', BinEdges_Length,...
        'BinPercentage_Length', BinPercentage_Length, 'BinNumber_Area', BinNumber_Area, 'BinEdges_Area', BinEdges_Area, ...
        'BinPercentage_Area', BinPercentage_Area);
