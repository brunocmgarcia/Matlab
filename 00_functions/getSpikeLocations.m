 function allLocs = getSpikeLocations(data,validMask,Fs,varargin)
    ...
    if exist('saveDir','var') && exist('savePrefix','var')
        ...
        defaultColors = parula(32);%get(gca,'colororder');
        legendText = {};
        validWireCount = 1;
        for ii=1:size(data,1)
            ...
            try plot(locsInSpan/Fs,zeros(1,length(locsInSpan)),'x','color','k','MarkerSize',10); end;
            xlim([0 max(t)]);
        ...
        for ii=1:size(data,1)
            ...
            savefig(fig,fullfile(saveDir,[savePrefix,'_waveforms',num2str(validWireCount)]),'compact');
            close(fig);
            validWireCount = validWireCount + 1;
        end
